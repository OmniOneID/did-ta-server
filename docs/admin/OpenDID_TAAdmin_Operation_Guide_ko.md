...
- [3. 기능별 상세 메뉴얼](#3-기능별-상세-메뉴얼)
  ...
  - [3.15. Admin Management](#315-admin-management)
  - [3.15.1 Admin 목록 조회](#3151-admin-목록-조회)
  - [3.15.2. Admin 등록](#3152-admin-등록)
...
---
> **참고:** 현재는 임시 Quick Register 방식만 지원되며, 정식 등록 절차는 2025년 6월에 업데이트될 예정입니다.
---
...
| 2    | **Name**              | TA의 이름입니다.                                                                                                                                       |
| 3    | **Status**            | TA가 등록 완료된 경우 COMPLETED 상태로 표시됩니다.                                                                                                     |
...
1. Entity 서버에서 DID Document를 생성합니다.  
2. TA 관리자가 해당 DID Document를 블록체인에 등록합니다.  
3. Entity 서버는 등록된 DID 정보를 기반으로 TA에 가입 증명서(VC)를 요청합니다.

---
> **참고:** 현재 Open DID 시스템에서는 위 등록 절차가 일부 간소화된 형태로 제공되고 있습니다.  
> 전체 절차에 따른 기능은 2025년 6월 중 정식으로 업데이트될 예정이며,  
> 그 전까지는 `QUICK REGISTER` 기능을 사용하는 것이 가장 원활한 방식입니다.
---
...
| 번호 | 항목                    | 설명                                                                  |
| ---- | ----------------------- | --------------------------------------------------------------------- |
| 1    | **REGISTER 버튼**       | 신규 Entity를 등록할 수 있는 버튼입니다.                              |
...
Entity의 DID Document를 블록체인에 등록합니다.   
기능 설명은 [3.2.2 Entity 등록](#322-entity-등록) 항목을 참고해 주세요.
...
---
> **참고:** 현재 Entity 등록은 테스트/개발 환경을 위한 `QUICK REGISTER` 기능 중심으로 제공됩니다.  
> 정식 등록 절차는 2025년 6월 중 도입될 예정입니다.
---

`REGISTER` 기능은 제한적인 수동 등록 방식으로,  
특정 서버만 개별 등록하거나 테스트 목적의 세부 조작이 필요한 경우에 사용할 수 있습니다.

`Entity Management` 화면에서 **REGISTER** 버튼을 클릭하면 아래와 같은 등록 화면으로 이동합니다

<img src="./images/3-5.entity-registration.png" width="600"/>

|번호| 항목                        | 설명                                                                                         |
|---| --------------------------- | -------------------------------------------------------------------------------------------- |
|1| **DID Document**            | 등록할 Entity의 DID Document 파일을 업로드합니다.                                            |
|2| **Name**                    | Entity의 이름을 입력합니다. 중복 여부는 `Check Availability` 버튼을 통해 확인할 수 있습니다. |
|3| **Role**                    | Entity의 역할을 선택합니다. 예: Issuer, Verifier, Wallet 등                                  |
|4| **URL**                     | Entity 서버의 호출 URL을 입력합니다. 예: `http://192.168.0.10:8091/issuer`                   |
|5| **Check Availability 버튼** | 입력한 이름이 중복되지 않는지 확인하는 버튼입니다.                                           |
|6| **Test Connection 버튼**    | 입력한 URL로 연결 테스트를 수행합니다.                                                       |
|7| **REGISTER**                | 입력한 정보를 기반으로 Entity를 등록합니다.                                                  |
|8| **RESET**                   | 모든 입력값을 초기화합니다.                                                                  |
|9| **CANCEL**                  | 등록 화면을 닫고 이전 페이지로 돌아갑니다.                                                   |
