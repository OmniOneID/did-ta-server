 ...
 */
package org.omnione.did.tas.v1.agent.config;
...
public class RepositoryRequestInterceptor implements RequestInterceptor {
    ...
    public void apply(RequestTemplate template) {
        try {
            ...
            log.error("OpenDidException while adding X-API-Key header: {}", e.getErrorCode(), e);
            throw e;
        } catch (org.springframework.dao.DataAccessException e) {
            log.error("Database access error while retrieving API key configuration", e);
            throw new OpenDidException(ErrorCode.SERVER_CONFIG_KEY_NOT_FOUND);
        } catch (Exception e) {
            log.error("Unexpected error while adding X-API-Key header to Repository request", e);
            throw new OpenDidException(ErrorCode.SERVER_CONFIG_KEY_NOT_FOUND);
        }
    }
}
